...
//        ChatGptDiaryResponseDto responseDto = restTemplate40.postForObject(API_URL, requestDto, ChatGptDiaryResponseDto.class);
     ...
     * @return : gpt4가 만들어준 대답 프롬프트
     */
    public List<Message> askGpt41Old(List<Message> messages, String prompt) throws Exception {
        Instant start = Instant.now();
...
//        ChatGptDiaryResponseDto responseDto = restTemplate40.postForObject(API_URL, requestDto, ChatGptDiaryResponseDto.class);
        ...
        if (!answer.endsWith("}")) {
            messages = askGpt41Old(messages, ChatGptPrompts.generateUserContinuePrompt());
        }
