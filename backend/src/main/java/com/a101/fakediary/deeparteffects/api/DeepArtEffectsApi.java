...
import com.a101.fakediary.deeparteffects.request.DeepArtEffectsRequest;
import com.a101.fakediary.deeparteffects.response.DeepArtEffectsResponse;
...
//                .bodyToMono(String.class);
//
//        //  Mono의 결과를 블로킹하여 String 형태로 반환
//        String responseBody = response.block();
//
//        //  JSON을 파싱하여 submissionId 값을 추출
//        JSONObject jsonObject = new JSONObject(responseBody);
//        String submissionId = jsonObject.getString("submissionId");
//
...
//    }
    ...
    public String uploadImageWithStyleId(MultipartFile origImageFile, String styleId) throws IOException {
        ...
        // POST 요청 보낼 body 생성
        DeepArtEffectsRequest request = new DeepArtEffectsRequest(styleId, base64EncodedImage);

        // WebClient로 POST 요청 보내기
        Mono<DeepArtEffectsResponse> response =  webClient.post()
                .uri("/upload")
                .body(BodyInserters.fromValue(request))
                .retrieve()
                .bodyToMono(DeepArtEffectsResponse.class);
//                .map(DeepArtEffectsResponse::getSubmissionId);
