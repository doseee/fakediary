...
class _DiaryDetailScreenState extends State<DiaryDetailScreen> {
  ...
  void initState() {
    ...
    _scrollController = ScrollController(); /// 스크롤 컨트롤러
  ...
  changeSelector() {
    var maxScrollValue = _scrollController.position.maxScrollExtent; /// 화면 전체 높이
    var scrollValue = _scrollController.offset.round(); ///현재 스크롤한 위치를 반올림한 값
    var divisor = (maxScrollValue / (details.length + 2)) + 80; /// 전체 크기를 화면구성요소 개수로 나누고 +80하며 슬라이드 하나가 차지할 크기를 계산
    var slideNum = (scrollValue / divisor).round(); ///현재 스크롤 위치를 기반으로 슬라이드 번호 계산
    ...
    var isExpanded = false; ///구분선 확장 관리 변수
    var isTransparent = false; /// SliverAppBar 투명도 관리 변수

    if(slideNum == 0){ /// 첫번째 슬라이드라면
      isExpanded = true; /// 구분선 확장 시작
    }else if(slideNum > 0 && slideNum <= details.length){ /// 두번째 슬라이드부터 마지막 디테일 슬라이드까지
      isTransparent = true; /// 투명도 상태 변경
      isExpanded = false; /// 구분선 상태 변경
    }
  ...
  String title (subtitles) {
    if(slideNum > 0 && slideNum <= details.length ) { /// 디테일 개수 안에 있는 서브타이틀
      return subtitles[slideNum-1]; ///표시
  ...
  Widget build(BuildContext context) {
    return WillPopScope(onWillPop: () async { /// 뒤로가기 방지
    ...
    child: Scaffold(
      body: FutureBuilder<DiaryModel>( /// API로 가져온 다이어리 모델 요소 사용
          future: diary,
          builder: (context, snapshot) {
            if (snapshot.hasData) { /// 데이터가 있다면
              details = snapshot.data!.detail.split('@'); ///details split 하여 미리 만들어둔 details 리스트로 갱신
              return contentList(snapshot.data!, details); /// 현재 diary 정보를 이용하여 화면 구성
            } else if (snapshot.hasError) { /// 정보가 없다면 에러 표시
              return Text("${snapshot.error}에러!!");
            }
            return CircularProgressIndicator(); /// 통신할 때 렌더링 중이라는 표시
          })
    ));
  }
  ...
  Widget CheckFriendLetter(){
    ...
    return SizedBox(
      height: 250,
      child: Padding(
        padding: const EdgeInsets.all(25.0), /// 상하좌우 패딩 25 적용
        child: Column(
          children: [
            Center(child: CheckCard()), /// 일기를 만드는 데 사용한 카드
            SizedBox( /// 중간 간격
              height: 30,
            ),
            ChangeButton( /// 교환 버튼
              exchangeSituation: widget.exchangeSituation, ///현재 교환 상태가 어떠한지 parameter로 넘김
              diaryId: widget.diaryId, /// 지금 상세페이지로 조회하고 있는 다이어리 아이디 parameter로 넘김
            )
          ],
        ),
      ),
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      /// (5) 폰트 이름 : 카페24 써라운드 에어
                        color: Colors.white, fontSize: 16, height: 1.5, letterSpacing: 1.1, fontFamily: 'Cafe24'),
                  ),
                ),
              ),
              Padding(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          Flexible(
                            flex: 4,
                            child: buildArtworkTitle(diary.title), /// 타이틀 스타일 위젯
                          ),
