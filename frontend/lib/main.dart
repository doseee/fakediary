...
sendAlarm(String title, String content, String flutterNotificationClick) async {
  final pref = await SharedPreferences.getInstance();
  String memberId = pref.getInt('memberId').toString();
  if (memberId == flutterNotificationClick) {
  ...
  FirebaseMessaging.onMessage.listen((RemoteMessage message) {
    ...
    if (message.data["body"] == '친구 맺고 일기를 교환해볼까요?') {
      ...
      sendAlarm(message.data["title"], message.data["body"],
          message.data["FLUTTER_NOTIFICATION_CLICK"]);
    }
    ...
    if (message.data["body"] == '친구 행성에서는 어떤 일기를 보냈을까요?') {
      print('일기교환완료');
      sendAlarm(message.data["title"], message.data["body"],
          message.data["FLUTTER_NOTIFICATION_CLICK"]);
    }
